...
export class BaseUser {
  ...
  /**
   * This function pastes text into the given selector using keyboard shortcuts.
   * @param selector - The CSS selector of the element to paste text into.
   */
  ...
  /**
   * Executes a flow within a modal dialog: waits for it to open,
   * performs actions, and verifies it closes.
   * @param selector - The CSS selector of the modal container.
   * @param whenOpened - Callback to execute when the modal is open.
   * @param beforeOpened - Optional callback executed before waiting for the modal.
   * @param afterClosing - Optional callback executed after the modal closes.
   */
  async doWithinModal({
    selector,
    beforeOpened = async (_this, container) => {
      await _this.expectElementToBeVisible(container);
    },
    whenOpened,
    afterClosing = async (_this, container) => {
      await _this.expectElementToBeVisible(container, false);
    },
  }: {
    selector: string;
    beforeOpened?: (user: BaseUser, container: string) => Promise<void>;
    whenOpened: (user: BaseUser, container: string) => Promise<void>;
    afterClosing?: (user: BaseUser, container: string) => Promise<void>;
  }): Promise<void> {
