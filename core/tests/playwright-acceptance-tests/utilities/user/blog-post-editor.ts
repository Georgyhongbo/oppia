...
export class BlogPostEditor extends BaseUser {
  ...
  async clickOnUpdateUsernameIcon(): Promise<void> {
    ...
    await this.expectModalTitleToBe(
   ...
   */
  async clickOnPasteValidComponentsButton(
    textPasted: string
  ): Promise<void> {
    await this.expectElementToBeVisible(pasteErrorBox);
   ...
   */
  async createNewBlogPostWithTitle(
    newBlogPostTitle: string
  ): Promise<void> {
    ...
    await this.updateBlogPostTitle(newBlogPostTitle);
    await this.updateBodyTextTo(
      'test blog post body content - duplicate'
    );
    await this.selectTag('News');
    await this.selectTag('International');
    await this.saveBlogBodyChanges();
   ...
   */
  async expectRemainingTagsLimitTextToBe(
    tagLimit: number
  ): Promise<void> {
...
export const BlogPostEditorFactory = (
  page: Page
): BlogPostEditor => new BlogPostEditor(page);
